...
if __name__ == "__main__":
    ...
    state_ranges = [[-4.8, 4,8], [-4, 4], [-0.418, 0.418], [-5, 5]]
    # num_of_actions = 30
    # action_range = [-2, 2]
    ...
    for i in range(num_of_episodes):
        ...
        while not done:
            ...
            for j, a in enumerate(actions):
                if j == last_action:
                    actor_weights[j, last_state_tiles] += actor_step_size * td_error * (1 - softmax_probability(last_state_tiles, j))
                else:
                    actor_weights[j, last_state_tiles] += actor_step_size * td_error * (0 - softmax_probability(last_state_tiles, j))
            ...
            if done:
                if i > 0 and i % 50 == 0:
                    ...
                    rewards_per_50_episodes.append(total_reward / 50)
                    total_reward = 0
            ...
            #     total_reward = 0
            #     done = True
    plt.plot(rewards_per_50_episodes)
    plt.xlabel("Episode")
