...
def create_bin_edges(ranges, num_of_bins):
    return np.array([np.linspace(ranges[0][0], ranges[0][1], num_of_bins + 1),
                         np.linspace(ranges[1][0], ranges[1][1], num_of_bins + 1),
                         np.linspace(ranges[2][0], ranges[2][1], num_of_bins + 1),
                         np.linspace(ranges[3][0], ranges[3][1], num_of_bins + 1)])
...
if __name__ == "__main__":
    q_table = np.random.uniform(low=0, high=1, size=([30, 30, 30, 30] + [env.action_space.n]))
    ...
    for i in range(1000):
        if i % 50 == 0:
            ...
            print(f"Average reward for past 50 episodes: {average_reward}")
            total_reward = 0
        obs = env.reset()
        done = False
        ...
        while not done:
            ...
            if not done:
                ...
                q_table[last_state, last_action] = (1 - step_size) * q_table[last_state, last_action] + (reward + gamma * max_next_state_action_value)
                ...
                q_table[last_state, last_action] = (1 - step_size) * q_table[last_state, last_action] + (-10)
        epsilon *= 0.99
